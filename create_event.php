...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Sanitize inputs (ensure sanitizeInput is defined)
  ...
  <form method="post" novalidate>
    ...
    <div class="form-row">
      <div class="form-group">
        ...
        <input type="datetime-local" id="start_date" name="start_date" required value="<?= htmlspecialchars($start_date) ?>">
      </div>

      <div class="form-group">
        <label for="end_date">End Date & Time</label>
        <input type="datetime-local" id="end_date" name="end_date" value="<?= htmlspecialchars($end_date) ?>">
      </div>
    </div>
    ...
    <div class="form-row">
      <div class="form-group">
        ...
        <select id="event_type" name="event_type" required>
          ...
          foreach ($types as $key => $label): ?>
            <option value="<?= $key ?>" <?= ($event_type === $key) ? 'selected' : '' ?>><?= $label ?></option>
          <?php endforeach; ?>
        </select>
      </div>
      ...
      <div class="form-group">
        ...
        <input type="number" id="capacity" name="capacity" min="1" required value="<?= htmlspecialchars($capacity) ?>">
      </div>
    </div>

    <button type="submit" class="btn btn-primary">Create Event</button>
    <a href="dashboard.php" class="btn btn-secondary" style="color:black;background-color:red">Cancel</a>
  </form>
</div>
